...
// GET - Return empty data (localStorage is used instead)
export async function GET(request: NextRequest) {
  // Return empty data - localStorage is used on client-side
...
export async function DELETE(request: NextRequest) {
  // Data is cleared from localStorage on client-side, not via API
  return NextResponse.json({ success: true, message: 'Using localStorage only' })
}
