...
const UPLOAD_NOTIFICATION_COOLDOWN_MS = 60_000
      ...
      if (response.ok) {
        ...
        if (systemNotifications.length > 0) {
          console.log(`[refreshNotifications] 📬 Found ${systemNotifications.length} System notification(s):`, systemNotifications.map((n: any) => n.message))
                ...
                // Normalize hazard messages (migrate older formats to the newest format)
        ...
        const normalizedNotifications = deduplicatedNotifications.map((n: any) => {
          const normalized = normalizeMessage(n.message || '')
          return normalized !== n.message ? { ...n, message: normalized } : n
        })

        // Persist normalized messages back to the DB (best-effort)
        const changed = normalizedNotifications.filter((n: any, idx: number) => n.message !== deduplicatedNotifications[idx]?.message)
        if (changed.length > 0) {
          console.log(`[refreshNotifications] ✍️ Normalizing ${changed.length} notification message(s)`) 
  ...
  useEffect(() => {
    ...
    const checkHazardSigns = async () => {
      try {
        ...
        const recentUploadResetAt =
          typeof window !== 'undefined' ? localStorage.getItem(NOTIFICATION_UPLOAD_RESET_AT_KEY) : null
        if (recentUploadResetAt) {
          const elapsedMs = Date.now() - new Date(recentUploadResetAt).getTime()
          if (!Number.isNaN(elapsedMs) && elapsedMs >= 0 && elapsedMs < UPLOAD_NOTIFICATION_COOLDOWN_MS) {
            console.log('[Notifications] Skipping hazard regeneration right after upload reset')
            return
          }
        }
        ...
        Array.from(workroomMap.values()).forEach((w) => {
          ...
          if (rescheduleRateScore < threshold && avgRescheduleRate != null && !isNaN(avgRescheduleRate)) {
            hazards.push({
              workroom: w.name,
              metric: 'Reschedule Rate',
              message: `⚠️ Reschedule Rate: ${avgRescheduleRate.toFixed(1)}% is above target`
            })
          }
        ...
        for (let i = 0; i < hazards.length; i++) {
          ...
          const hazardMessage = (hazard.message || '').toString().trim()
          const isSameMessage = existingMessage === hazardMessage

          if (existingNotification && isSameMessage) {
            console.log(`[Notifications] ⛔ Skipping ${hazard.metric} notification for ${hazard.workroom}: no change`)
            continue
          }

          console.log(
            existingNotification
              ? `[Notifications] 🔁 Updating hazard notification: ${hazard.workroom} - ${hazard.metric} - ${hazardMessage}`
              : `[Notifications] ✅ Creating hazard notification: ${hazard.workroom} - ${hazard.metric} - ${hazardMessage}`
          )
